...
const CampaignDetailsModal: React.FC<{ campaign: FullCampaign; isOpen: boolean; onClose: () => void; onJoinSuccess: () => void; }> = ({ campaign, isOpen, onClose, onJoinSuccess }) => {
  ...
  const handleJoin = async () => {
    try {
        ...
        onJoinSuccess(); // This will tell the Campaigns page to refetch data
        onClose(); 
    } catch (err) { console.error("Failed to join campaign", err); }
  };
  ...
  const renderJoinButton = () => {
    if (isParticipant) {
        return <button onClick={navigateToCampaignPage} className="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-3 rounded-lg transition-colors">View Campaign Page</button>;
    }
    if (isOnWaitlist) {
    ...
    <Modal isOpen={isOpen} onClose={onClose} title={campaign.name}>
      <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-2 text-gray-300">
        ...
        <div className="flex items-center space-x-4">
            {campaign.platforms.map(p => ( <div key={p} className="flex items-center space-x-2 text-2xl text-white"> {platformIcons[p]} <span className="text-base">{p}</span> </div> ))}
        </div>
        ...
        {campaign.assets && campaign.assets.url && (
            <>
                ...
                <a href={campaign.assets.url} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline font-semibold"> {campaign.assets.name || 'Download Assets'} </a>
            </>
        )}
      </div>
    </Modal>
