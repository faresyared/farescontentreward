...
const Navbar = () => {
  ...
  const mobileNavLinkClass = ({ isActive }: { isActive: boolean }) => `flex flex-col items-center justify-center w-full py-2 rounded-lg text-xs font-medium transition-colors duration-300 ${ isActive ? 'text-red-400' : 'text-gray-400' }`;
    ...
    <>
      ...
      <nav className="sticky top-0 z-40 bg-black/70 backdrop-blur-xl border-b border-gray-800/50">
        <div className="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            {/* --- THIS IS THE FIX --- */}
            {/* Right Side: Profile Dropdown (ONLY visible on DESKTOP) */}
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-black/80 backdrop-blur-md border-t border-gray-800/50 p-1">
        <div className="flex justify-around items-center">
            <NavLink to="/dashboard/home" className={mobileNavLinkClass}><HomeIcon className="h-6 w-6" /> Home</NavLink>
            <NavLink to="/dashboard/campaigns" className={mobileNavLinkClass}><MegaphoneIcon className="h-6 w-6" /> Campaigns</NavLink>
            <NavLink to="/dashboard/earnings" className={mobileNavLinkClass}><CurrencyDollarIcon className="h-6 w-6" /> Earnings</NavLink>
            
            {/* The Profile Dropdown, but styled for the mobile bar */}
