...
const SignUp = () => {
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // --- THIS IS THE CHANGE ---
      // We don't log in. We just show the success message from the backend.
    ...
    <div className="min-h-screen bg-black text-gray-300 flex items-center justify-center p-4 font-sans">
      ...
      <div className="relative w-full max-w-md bg-black/50 backdrop-blur-lg rounded-2xl border border-gray-700/50 shadow-2xl shadow-red-500/10">
        <div className="p-8">
          ...
          {/* --- NEW: Show success message instead of the form --- */}
          {successMessage ? (
            <div className="text-center py-8">
              <p className="text-green-400 font-bold text-lg">{successMessage}</p>
              <p className="text-gray-300 mt-2">You can now close this page.</p>
            </div>
          ) : (
            <>
